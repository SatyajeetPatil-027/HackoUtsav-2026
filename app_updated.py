...
def get_final_text(verdict):
    if not verdict:
        return ""
    if hasattr(verdict, "final_text"):
        return verdict.final_text
    if isinstance(verdict, dict):
        if "repaired_text" in verdict:
            return verdict["repaired_text"]
        if "selected_output" in verdict:
            return verdict.get("selected_output", {}).get("repaired_text", "")
    return ""


def get_agent_scores(verdict):
    if not verdict:
        return []
    if hasattr(verdict, "all_iterations"):
        iterations = verdict.all_iterations
    elif isinstance(verdict, dict):
        iterations = verdict.get("all_iterations", [])
    else:
        return []
    if not iterations:
        return []
    return iterations[-1].get("scored_outputs", [])
...
with col_left:
    ...
    input_mode = st.radio(
        "Input method",
        ["Upload Audio File", "Record from Mic", "Load from Path"],
        horizontal=True,
    ...
    elif input_mode == "Load from Path":
        path = st.text_input("Enter audio file path", value=r"C:\Users\Aditya\Downloads\Dataset (1).zip\audio")
        if path:
            p = Path(path)
            if p.exists():
                if p.is_file():
                    ...
                    st.audio(audio_bytes)
                    st.session_state.raw_audio = audio_bytes
                    st.session_state._upload_filename = p.name
                else:
                    st.error("The path points to a directory, not a file. Please provide a file path.")
            else:
                st.warning("File does not exist at the given path.")
    else:
    ...
    # ── ASR + NLP Tokenization ─────────────────
    st.markdown(
        '<div class="section-card"><div class="section-title">🧠 ASR + NLP Tokenization</div>',
        unsafe_allow_html=True,
    )

    run_transcription = st.button(
        "🧠 Transcribe Audio + Generate Tokens",
        disabled=not bool(st.session_state.raw_audio),
        use_container_width=True,
    )

    if run_transcription:
        with st.spinner("Transcribing audio and generating tokens…"):
            try:
                os.makedirs("audio", exist_ok=True)
                os.makedirs("transcripts", exist_ok=True)
                os.makedirs("tokens", exist_ok=True)

                filename = st.session_state.get("_upload_filename", "uploaded_audio.wav")
                audio_path = os.path.join("audio", filename)

                with open(audio_path, "wb") as f:
                    f.write(st.session_state.raw_audio)
                ...
                try:
                    ...
                    from audio_to_text_converter import transcribe_audio
                    raw_text = transcribe_audio(audio_path)

                try:
                    from nlp_cleaner import clean_asr_output
                    report = clean_asr_output(raw_text)
                    cleaned_text = report.cleaned
                ...
                try:
                    from nlp_tokenizer import tokenize_text
                    tokens = tokenize_text(cleaned_text)
                except Exception:
                    tokens = cleaned_text.split()

                Path("transcripts/transcript.txt").write_text(raw_text, encoding="utf-8")
                Path("tokens/tokens.txt").write_text("\n".join(tokens), encoding="utf-8")

                st.session_state.raw_text = raw_text
                st.session_state.cleaned_text = cleaned_text
                st.session_state.tokens = tokens
                st.session_state.stage = "cleaned"

                st.success("✅ Transcription and tokenization completed!")
                st.rerun()

            except Exception as e:
                st.session_state.error = f"Transcription/tokenization failed: {e}"
    ...
    if st.session_state.cleaned_text:
        ...
        st.markdown(
            f'<div class="text-block cleaned">{st.session_state.cleaned_text}</div>',
            unsafe_allow_html=True,
        )
        st.caption(f"Tokens generated: {len(st.session_state.tokens)}")
...
with col_right:
    ...
    st.markdown(
        '<div class="section-card"><div class="section-title">🤖 Master Repair</div>',
        unsafe_allow_html=True,
    )

    run_master = st.button(
        "🚀 Run Master Agent Repair",
        disabled=not bool(st.session_state.tokens),
        use_container_width=True,
    )

    if run_master:
        with st.spinner("Running master repair…"):
            try:
                from master_agent import MasterAgent

                master = MasterAgent()
                verdict = master.run(st.session_state.tokens)
                st.session_state.repair_verdict = verdict
                st.session_state.stage = "repaired"
                st.rerun()
            except Exception as e:
                st.session_state.error = f"Master Agent failed: {e}"

    if verdict is not None:
        final_text = get_final_text(verdict)
        st.markdown(
            f'<div class="text-block repaired">{final_text}</div>',
            unsafe_allow_html=True,
        )

        scored_outputs = get_agent_scores(verdict)
        if scored_outputs:
            st.markdown("**Agent Scores:**")
            for item in scored_outputs:
                score = item.get("scores", {}).get("final_score", 0)
                st.write(f"{item.get('agent_name', 'agent')}: {score:.3f}")
    else:
        st.markdown(
            '<div class="text-block empty">No master repair output yet.</div>',
            unsafe_allow_html=True,
        )
    ...
    final_text = get_final_text(verdict) if verdict else ""
...
if st.session_state.raw_text and (st.session_state.repair_verdict or st.session_state.cleaned_text):
    ...
    with c2:
        final = (
            get_final_text(st.session_state.repair_verdict)
